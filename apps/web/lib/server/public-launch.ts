...
import { HOODED_GENESIS_MANIFEST, type LaunchManifestV1 } from "@hooded/shared";
...
export const getPublicLaunch = cache(async (projectId: string): Promise<LaunchManifestV1 | null> => {
  if (projectId === HOODED_GENESIS_MANIFEST.metadata.projectId && !databaseConfigured()) return HOODED_GENESIS_MANIFEST;
