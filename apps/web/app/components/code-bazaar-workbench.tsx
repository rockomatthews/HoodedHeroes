  ...
  async function startSandbox() {
    ...
    try {
      const response = await fetch("/api/sandbox/sessions", {
        ...
        body: JSON.stringify({ repository: "rockomatthews/HOODED", runtime: "web-evm-v1" }),
      });
  ...
  return (
    <div className="bazaar-lab bazaar-lab--production">
      ...
      {view === "workbench" ? <div className="bazaar-workspace">
        ...
        <div className="bazaar-output"><span>{`REPRODUCIBLE EVIDENCE // ${selectedWorkstream.stage}`}</span>{runCount === 0 ? <><p>Local policy preview ready. A live run requires a verified Genesis Hero session.</p>{selectedWorkstream.checks.map((check) => <p key={check}>{`○ REQUIRED // ${check}`}</p>)}</> : checks.map(([label, ok]) => <p key={label} className={ok ? "test-pass" : "test-fail"}>{ok ? "✓" : "×"} {label}</p>)}<div className="evidence-strip"><b>DIFF</b><b>SBOM</b><b>BUILD HASH</b><b>PR</b></div></div>
