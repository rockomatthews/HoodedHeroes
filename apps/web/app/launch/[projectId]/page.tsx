...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${launch.metadata.name} (${launch.metadata.symbol}) — HoodedHeroes Launch Bay`,
    description: launch.metadata.publication.summary,
    alternates: { canonical: launch.metadata.canonicalLaunchUrl },
    openGraph: { title: `${launch.metadata.name} // ${launch.metadata.symbol}`, description: launch.metadata.publication.summary, type: "website", images: [{ url: "/og.png", width: 1200, height: 630, alt: `${launch.metadata.name} HoodedHeroes launch` }] },
...
export default async function PublicLaunchPage({ params }: Props) {
  ...
  const structuredData = {
    ...
    isPartOf: { "@type": "WebSite", name: "HoodedHeroes" },
  };
  ...
  return (
    <main className="public-launch-shell">
      ...
      <section className="public-launch-card">
        <header className="public-launch-header"><Link href="/" aria-label="Return to HoodedHeroes">← HQ</Link><span>LAUNCH BAY // PUBLIC VESTIBULE</span><b>{launch.environment.toUpperCase()}</b></header>
        <div className="public-launch-hero">
          <div className="public-token-emblem"><i>H</i></div>
          <div><small>ROBINHOOD CHAIN // FIXED SUPPLY</small><h1>{projectId === "hoodedheroes-hero-genesis" ? "Hooded Heroes" : launch.metadata.name}</h1><h2>${launch.metadata.symbol}</h2><p>{launch.metadata.publication.summary}</p></div>
        ...
        <div className="public-launch-grid">
          ...
          <section><h3>THE ACCESS SEQUENCE</h3><ol><li><b>01</b><span>Audits, legal controls, and testnet exercises</span></li><li><b>02</b><span>Public vestibule fair-launch window</span></li><li><b>03</b><span>25,000 HERO unlocks society preview</span></li><li><b>04</b><span>A Genesis Hero unlocks builder access</span></li></ol></section>
        </div>
