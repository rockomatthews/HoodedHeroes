...
export async function GET(_request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [launches, totals, liquidity, events] = await Promise.all([
    ...
    sql`select coordinator_address, lock_address, position_manager, position_id, quote_amount, token_amount, permanently_locked, verified_at from liquidity_positions where project_id = ${projectId} limit 1`,
