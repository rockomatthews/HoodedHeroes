...
export async function POST(request: Request) {
  try {
    ...
    const configuredFactory = manifest.metadata.chain === "robinhood"
      ? (manifest.launchClass === "production" ? process.env.RH_PRODUCTION_LAUNCH_FACTORY_ADDRESS : process.env.RH_LAUNCH_FACTORY_ADDRESS)
      : null;
    if (manifest.metadata.chain === "robinhood" && (!configuredFactory || !isAddress(configuredFactory))) {
      return Response.json({ error: "Verified launch factory is not configured" }, { status: 503 });
    }
    const sql = db();
    ...
    await sql.begin(async (tx) => {
      await tx`insert into launches (project_id, creator_wallet, chain, environment, launch_class, lifecycle, token_address, factory_address, manifest, manifest_hash) values (${manifest.metadata.projectId}, ${session.wallet.toLowerCase()}, ${manifest.metadata.chain}, ${manifest.environment}, ${manifest.launchClass}, ${manifest.lifecycle}, ${manifest.metadata.tokenAddress ?? null}, ${configuredFactory ? getAddress(configuredFactory).toLowerCase() : null}, ${tx.json(manifest)}, ${hash})`;
