...
const bodySchema = z.object({ runtime: z.enum(["web-evm-v1", "solana-v1"]) });
...
export async function POST(request: Request) {
  try {
    ...
    await requireDatabaseRateLimit("sandbox-create", session.wallet, 6, 3_600);
    ...
    const created = await createCommunitySandbox({ ...body, repository, baseCommit });
    await sql`insert into sandbox_sessions (id, idempotency_key, owner_wallet, repository, base_commit, runtime_image, provider_session_id, status, preview_url, expires_at) values (${created.id}, ${key}, ${session.wallet.toLowerCase()}, ${repository}, ${baseCommit}, ${body.runtime}, ${created.providerSessionId}, ${created.status}, ${created.previewUrl}, ${created.expiresAt})`;
