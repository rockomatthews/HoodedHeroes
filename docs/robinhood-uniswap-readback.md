...
The HOODED v1.6 v4 adapter implements exact sale-price initialization, an adapter-owned `beforeInitialize` guard, a full-range position, Permit2 settlement and same-transaction allowance cleanup, permanent NFT receipt, and forced-balance-safe residue removal. A local QuickNode-backed fork must exercise the adapter and complete sale-to-pool path against the deployed contracts before release. Source, bytecode, and fork evidence still require independent review.
