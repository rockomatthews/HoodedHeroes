...
The v1.5 Robinhood Uniswap v4 adapter is implemented as a no-hook, full-range position using the canonical PoolManager, PositionManager, Permit2, and WETH contracts. It has no callback entry point, rejects an existing pool unless its price exactly matches the sale-derived price, uses explicit maximum inputs, clears both allowance layers, and empties its balances. The coordinator independently verifies pool ID, price, range, exact expected liquidity, NFT ownership, and the permanent receiver. Implementation is not deployment approval: independent adapter review, frozen bytecode, runtime-hash readback, and the approved fork canary remain release gates.
