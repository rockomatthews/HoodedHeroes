...
Important license boundary: the StonkBrokers documentation identifies its protocol and launchpad code as BUSL 1.1 and explicitly withholds competitive production-use rights before its change date. HOODED may study public behavior and integrate public interfaces where permitted, but must not copy or deploy its licensed contracts.
...
The useful lesson for HOODED is the intent model: builders sign a bounded proposal, assets and authority remain with the signer until execution, and every executor must honor the signed floor. That maps well to launch proposals, review approvals, and future treasury actions.
...
| Durable member identity | A Genesis Hero grants builder access and carries reputation; transfer resets earned progression |
