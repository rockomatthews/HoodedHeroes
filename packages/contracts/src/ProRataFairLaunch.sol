...
contract ProRataFairLaunch is ReentrancyGuard {
    ...
    function claim() external nonReentrant {
        ...
        uint256 accepted = totalContributed > maximumRaise ? maximumRaise * contribution / totalContributed : contribution;
        ...
        if (refundAmount > 0) _payQuote(msg.sender, refundAmount);
        emit Claimed(msg.sender, tokens, accepted, refundAmount);
    }
    ...
    function preview(address contributor) external view returns (uint256 tokens, uint256 accepted, uint256 refundAmount) {
        uint256 contribution = contributed[contributor];
        uint256 denominator = totalContributed > maximumRaise ? totalContributed : maximumRaise;
        tokens = denominator == 0 ? 0 : saleAllocation * contribution / denominator;
        accepted = totalContributed > maximumRaise && totalContributed > 0 ? maximumRaise * contribution / totalContributed : contribution;
    ...
    function _recordContribution(address contributor, uint256 amount, address referrer) internal {
        require(!cancelled && !paused, "not accepting");
    ...
    function _isVerifiedReferrer(address referrer) internal view returns (bool) {
        ...
        (bool ok, bytes memory data) = referralRegistry.staticcall(abi.encodeWithSignature("isVerified(address)", referrer));
