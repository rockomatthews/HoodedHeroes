...
contract HoodedHeroesGenesis is ERC721, ReentrancyGuard {
    ...
    IERC20 public immutable heroToken;
    ...
    constructor(address token, address rewards, address treasury) ERC721("HoodedHeroes Genesis", "HHERO") {
        require(token != address(0) && rewards != address(0) && treasury != address(0), "zero address");
        heroToken = IERC20(token);
    ...
    function mint(Tier tier) external nonReentrant returns (uint256 tokenId) {
        ...
        require(heroToken.transferFrom(msg.sender, address(this), price), "payment failed");

        uint256 burnAmount = price * 40 / 100;
        ERC20Burnable(address(heroToken)).burn(burnAmount);
        require(heroToken.transfer(seasonalRewards, price * 40 / 100), "reward transfer failed");
        require(heroToken.transfer(daoTimelock, price * 20 / 100), "treasury transfer failed");
